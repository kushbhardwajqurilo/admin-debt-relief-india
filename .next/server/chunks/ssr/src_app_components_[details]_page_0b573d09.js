...
function UserDashboard() {
    ...
    const checkPayment = async (subIndex)=>{
        try {
            const res = await fetch(`/api/checkPayment?month=${user.unpaidSubscriptions[subIndex].paidForMonth}`);
            ...
            if (data.status === "paid") {
                ...
                alert(`Subscription for ${data.paidForMonth} is now PAID.`);
            } else {
                alert(`Subscription for ${user.unpaidSubscriptions[subIndex].paidForMonth} is still PENDING.`);
            }
        } catch (err) {
            console.error(err);
    ...
    const markAsPaid = (subIndex)=>{
        ...
        alert(`Subscription for ${user.unpaidSubscriptions[subIndex].paidForMonth} marked as PAID.`);
    };
...
//# sourceMappingURL=src_app_components_%5Bdetails%5D_page_0b573d09.js.map